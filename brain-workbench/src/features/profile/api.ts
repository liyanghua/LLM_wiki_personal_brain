...
export async function getMethodProfile() {
  const payload = await apiClient.get(ENDPOINTS.profileMethod, () => methodProfileMock);
  return mapMethodProfile(payload);
}

export async function getPersistentMemory() {
  return await apiClient.get(ENDPOINTS.profileMemory, () => persistentMemoryMock);
}

export async function getProfileProposals() {
  return await apiClient.get(ENDPOINTS.profileProposals, () => profileProposalsMock);
}
