...
function wikiRefsForType(source: StrategySourceBundle, type: StrategyCardType): string[] {
...
function sourceTextForType(source: StrategySourceBundle, type: StrategyCardType, sourceKeys: string[]): string {
  const wikiText = (HERO_WIKI_PAGE_BY_TYPE[type] ?? [])
    .map((pageName) => source.wikiPages[pageName] ?? "")
    .filter(Boolean)
    .join("\n\n")
...
async function loadStrategySourceBundle(projectPath: string, report: AgentModeReport): Promise<StrategySourceBundle> {
  ...
  const businessRoot = `${pp}/wiki/business/${docSlug}`
  const pageNames = ["index.md", "人群与场景.md", "卖点与表达.md", "素材与版式.md", "指标与判断.md", "动作与实验.md", "证据与案例.md"]
  const wikiPages: Record<string, string> = {}
  for (const pageName of pageNames) {
    const path = `${businessRoot}/${pageName}`
...
function buildActionCards(
  ...
  for (const type of types) {
    ...
    const wikiRefs = wikiRefsForType(source, type)
...
export async function writeStrategyBundle(
  ...
  report: AgentModeReport,
  scenePack: ScenePack,
  options: { llmConfig?: LlmConfig | null; signal?: AbortSignal } = {},
  ...
  const summaryPath = `${pp}/wiki/business/${source.docSlug}/策略总览.md`
  await createDirectory(`${pp}/wiki/business/${source.docSlug}`).catch(() => {})
