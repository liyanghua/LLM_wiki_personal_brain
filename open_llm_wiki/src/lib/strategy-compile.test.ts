...
function taskReport(): AgentModeReport {
  const now = "2026-05-20T00:00:00.000Z"
  const pack: TaskContextPack = {
    schemaVersion: "task_context_pack_v1",
    packId: "tcp-test",
    sourceDocId: "doc-task",
    sourceName: "经营周会.xlsx",
    sourcePath: "/tmp/经营周会.xlsx",
    sourceKind: "generic",
    docRole: "meeting_task_source",
    operatingGoals: [{ text: "提升周度经营目标达成率", evidenceRefs: ["goal-ref"] }],
    roleProfiles: [
      {
        roleName: "运营",
        responsibilities: ["负责周会任务拆解"],
        kpis: ["周目标达成率"],
        collaboratorRoles: ["美工"],
        evidenceRefs: ["role-ref"],
      },
    ],
    taskTriggers: [{ text: "周会形成行动项后需要下发任务", evidenceRefs: ["trigger-ref"] }],
    taskCandidates: [
      {
        taskId: "task-1",
        title: "优化主图点击率",
    ...
    metricRules: [{ text: "点击率提升10%才算通过", evidenceRefs: ["metric-ref"] }],
    collaborationRules: [{ text: "运营主责，美工协同交付素材", evidenceRefs: ["collab-ref"] }],
    reviewRules: [{ text: "周会复盘动作前后数据", evidenceRefs: ["review-ref"] }],
    evidenceAnchors: ["goal-ref", "task-ref", "metric-ref"],
    qualityWarnings: [],
  }
  return {
    ...baseReport(),
    ...
    understanding: {
      ...baseReport().understanding,
      title: "经营周会",
      summary: "将周会行动项编译为经营任务卡。",
    },
    groundTruth: {
      ...baseReport().groundTruth,
      docId: "doc-task",
      sceneId: "ecom_growth_task_generation",
      title: "经营周会",
      fields: [
        ...
        },
        {
          key: "task_trigger",
          label: "任务触发",
          value: "周会形成行动项后需要下发任务",
          evidenceBlockRefs: ["trigger-ref"],
          status: "confirmed",
          lastUpdatedAt: now,
          updatedFromCardId: null,
          acceptedPatchIds: [],
    ...
    compileIr: {
      ...baseReport().compileIr,
      sourceSummary: "经营周会任务生成",
      taskContextPack: pack,
      fieldValueMap: {
        operating_goal: "提升周度经营目标达成率",
        task_trigger: "周会形成行动项后需要下发任务",
      },
      fieldLabelMap: {
        operating_goal: "经营目标",
        task_trigger: "任务触发",
      ...
      fieldEvidenceMap: {
        operating_goal: ["goal-ref"],
        task_trigger: ["trigger-ref"],
      },
    },
    supportingWikiPages: [
      "wiki/tasks/经营周会.md",
      "wiki/roles/经营周会.md",
      "wiki/quality/经营周会.md",
      "wiki/collaboration/经营周会.md",
      "wiki/reviews/经营周会.md",
    ],
    taskContextPack: pack,
  }
}
...
describe("writeStrategyBundle", () => {
  ...
  it("writes task-generation strategy summaries under task pages instead of wiki/business", async () => {
    const tmp = await createTempProject("task-strategy-compile")
    cleanup = tmp.cleanup
    const report = taskReport()
    const scenePack = await ensureScenePack(tmp.path, { persistDefaults: true, defaultLanguage: "Chinese" })
    await writeSceneCompile(tmp.path, report, scenePack)

    const { bundle, writtenPaths } = await writeStrategyBundle(tmp.path, report, scenePack)

    expect(writtenPaths).toContain(`${tmp.path}/wiki/tasks/经营周会-策略总览.md`)
    expect(bundle.linkedWikiRefs).toContain("wiki/tasks/经营周会.md")
    expect(bundle.linkedWikiRefs).not.toContain("wiki/business/经营周会/index.md")
    expect(await readFileRaw(`${tmp.path}/wiki/tasks/经营周会-策略总览.md`)).toContain("策略总览")
  })
