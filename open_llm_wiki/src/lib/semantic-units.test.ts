...
describe("semantic unit index", () => {
  it("extracts semantic units from task context pack task cards and metrics", () => {
    const report = makeReport({
      ...
      taskContextPack: {
        ...
        sourceName: "周会.xlsx",
        sourcePath: "/tmp/周会.xlsx",
        sourceKind: "generic",
        docRole: "meeting_task_source",
        operatingGoals: [{ text: "提升转化率", evidenceRefs: ["goal-ref"] }],
        roleProfiles: [{
          roleName: "运营",
        ...
        taskTriggers: [{ text: "点击率下滑", evidenceRefs: ["trigger-ref"] }],
        taskCandidates: [{
          taskId: "task-1",
          title: "优化主图细节",
          trigger: "点击率下滑",
        ...
        metricRules: [{ text: "点击率提升10%", evidenceRefs: ["metric-ref"] }],
        collaborationRules: [{ text: "运营 协同 美工", evidenceRefs: ["collab-ref"] }],
        reviewRules: [{ text: "周会复盘", evidenceRefs: ["review-ref"] }],
        evidenceAnchors: ["task-ref"],
        qualityWarnings: [],
      },
      compileIr: {
    ...
    expect(index.units.some((unit) => unit.unitType === "action_recommendation" && unit.canonicalText.includes("优化主图细节"))).toBe(true)
    expect(index.units.some((unit) => unit.unitType === "metric_definition" && unit.canonicalText.includes("点击率提升10%"))).toBe(true)
    expect(index.units.some((unit) => unit.unitType === "rule" && unit.canonicalText.includes("运营 协同 美工"))).toBe(true)
  })
