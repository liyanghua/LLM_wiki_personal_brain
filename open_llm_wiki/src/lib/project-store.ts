...
import type { LlmConfig, SearchApiConfig, EmbeddingConfig, MultimodalConfig, OutputLanguage, ProviderConfigs, ProxyConfig, PdfBackendMode } from "@/stores/wiki-store"
...
export async function saveSearchApiConfig(config: SearchApiConfig): Promise<void> {
  const store = await getStore()
  const persisted: SearchApiConfig = {
    ...config,
    runtimeFirecrawlApiKey: "",
  }
  await store.set(SEARCH_API_KEY, persisted)
}
...
const PDF_BACKEND_KEY = "pdfBackendMode"

export async function savePdfBackendMode(mode: PdfBackendMode): Promise<void> {
  const store = await getStore()
  await store.set(PDF_BACKEND_KEY, mode)
}

export async function loadPdfBackendMode(): Promise<PdfBackendMode | null> {
  const store = await getStore()
  return (await store.get<PdfBackendMode>(PDF_BACKEND_KEY)) ?? null
}
