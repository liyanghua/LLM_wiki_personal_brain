...
import type { RootCause } from "@/lib/quality-contracts"
...
export interface LintResult {
  ...
  issueScope: "wiki_asset" | "wiki_compile" | "retrieval_runtime"
  rootCause: RootCause
  blocking: boolean
...
function issueScopeForType(_type: LintResult["type"]): LintResult["issueScope"] {
  return "wiki_asset"
}

function rootCauseForStructuralType(type: Exclude<LintResult["type"], "semantic">): RootCause {
  switch (type) {
    case "orphan":
      return "wiki_orphan_page"
    case "broken-link":
      return "wiki_broken_link"
    case "no-outlinks":
    default:
      return "wiki_no_outlinks"
  }
}

function rootCauseForSemanticRawType(rawType: string): RootCause {
  switch (rawType) {
    case "contradiction":
      return "wiki_contradiction"
    case "stale":
      return "wiki_stale_claim"
    case "missing-page":
      return "wiki_missing_topic_page"
    case "suggestion":
    default:
      return "wiki_weak_summary"
  }
}
...
export async function runStructuralLint(projectPath: string): Promise<LintResult[]> {
  ...
  for (const p of pages) {
    ...
    if (inbound === 0) {
      results.push({
        issueId: `lint-orphan-${shortName}`,
        type: "orphan",
        severity: "info",
        page: shortName,
        detail: "No other pages link to this page.",
        issueScope: issueScopeForType("orphan"),
        rootCause: rootCauseForStructuralType("orphan"),
        blocking: false,
        linkedDocId: null,
        linkedIssueIds: [],
      })
    ...
    if (p.outlinks.length === 0) {
      results.push({
        issueId: `lint-no-outlinks-${shortName}`,
        type: "no-outlinks",
        severity: "info",
        page: shortName,
        detail: "This page has no [[wikilink]] references to other pages.",
        issueScope: issueScopeForType("no-outlinks"),
        rootCause: rootCauseForStructuralType("no-outlinks"),
        blocking: false,
        linkedDocId: null,
        linkedIssueIds: [],
      })
    ...
    for (const link of p.outlinks) {
      ...
      if (!exists) {
        results.push({
          issueId: `lint-broken-${shortName}-${lookup}`,
          type: "broken-link",
          severity: "warning",
          page: shortName,
          detail: `Broken link: [[${link}]] — target page not found.`,
          issueScope: issueScopeForType("broken-link"),
          rootCause: rootCauseForStructuralType("broken-link"),
          blocking: true,
          linkedDocId: null,
          linkedIssueIds: [],
        })
...
export async function runSemanticLint(
  ...
  for (const match of matches) {
    ...
    const rootCause = rootCauseForSemanticRawType(rawType)

    results.push({
      issueId: `lint-semantic-${rawType}-${title.replace(/\s+/g, "-").toLowerCase()}`,
      type: "semantic",
      ...
      rootCause,
      blocking: rootCause === "wiki_contradiction",
      linkedDocId: null,
      linkedIssueIds: [],
    })
  }
