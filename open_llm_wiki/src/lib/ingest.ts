...
import { buildStructuredContext, runStep15Structuring } from "@/lib/structuring"
...
async function autoIngestImpl(
  ...
  const multimodalConfig = useWikiStore.getState().multimodalConfig
  const multimodalAvailable = Boolean(
    ...
    try {
      ...
      if (savedImages.length > 0) {
        ...
        if (!mmCfg.enabled) {
          ...
          if (captionLlm) {
            try {
              await captionMarkdownImages(pp, analysisBaseContent, captionLlm, {
                signal,
                shouldCaption: (url) =>
  ...
  let enrichedSourceContent = analysisBaseContent
  const mmCfg = useWikiStore.getState().multimodalConfig
  const captionLlm = resolveCaptionConfig(mmCfg, llmConfig)
  if (!mmCfg.enabled && savedImages.length > 0) {
    ...
    enrichedSourceContent = analysisBaseContent.replace(
      /!\[[^\]]*\]\([^)\s]+\)/g,
      " ",
    )
    console.log(
  ...
  activity.updateItem(activityId, {
    detail: isPdf
      ? preparedDocument?.backendStatus.degraded
        ? `增强识别 PDF 结构已降级，当前使用 ${preparedDocument?.backendStatus.mode ?? preferredPdfBackend} 文本链路...`
        : `增强识别 PDF 结构中（${preparedDocument?.backendStatus.mode ?? preferredPdfBackend}）...`
      : sourceKind === "doc"
        ? "转换老版 DOC 文档并解析结构..."
        : sourceKind === "docx"
          ? "解析 DOCX 结构与证据..."
  ...
  await appendQualityDraftReport(
    pp,
    structuredReport.docId,
    structuredReport.sourceName,
    structuredReport.qualityScore,
    structuredReport.qualitySummary,
  )
  const workingDraft = await ensureWorkingDraft(
    pp,
    structuredReport.docId,
    structuredReport.sourcePath,
    sourceContent,
  )
  useAgentModeStore.getState().upsertReport(structuredReport)
  useAgentModeStore.getState().setSelectedDocId(structuredReport.docId)
  useRevisionStore.getState().upsertDraft(workingDraft)
  ...
  const compiledReport: AgentModeReport = {
    ...
    compileSidecar: {
      ...
      compilePlan: compileResult.plan,
      compileCoverage: compileResult.compileCoverage,
      structuredContext: buildStructuredContext(structuredReport, scenePack),
    },
  }
  ...
  if (writeWarnings.length > 0) {
    const summary = writeWarnings.length === 1
      ? writeWarnings[0]
      : `${writeWarnings.length} 条编译提示：${writeWarnings.slice(0, 2).join(" · ")}${writeWarnings.length > 2 ? ` … (+${writeWarnings.length - 2} 更多)` : ""}`
  ...
  const detail = writtenPaths.length > 0
    ? `${writtenPaths.length} files written${reviewItems.length > 0 ? `, ${reviewItems.length} review item(s)` : ""} · structuring ${structuredReport.qualityScore}/100`
    : "No files generated"
...
function buildReviewItemsFromAgentReport(
  report: AgentModeReport,
  sourcePath: string,
): Omit<ReviewItem, "id" | "resolved" | "createdAt">[] {
  return report.revisionIssueCards.slice(0, 8).map((card) => ({
    type: card.severity === "P1" ? "confirm" : "suggestion",
    title: card.issueTitle,
    description: `${card.diagnosis}\n\n建议改写：\n${card.suggestedRevision}`,
    sourcePath,
    affectedPages: report.supportingWikiPages,
    searchQueries: [
      card.issueTitle,
      card.targetFieldKey ?? report.sourceName,
      report.understanding.mainlineSteps[0] ?? report.sourceName,
    ].filter(Boolean),
    options: [
      { label: "去业务修订", action: "goto-agent-mode" },
      { label: "稍后处理", action: "Skip" },
    ],
    origin: "agent_mode",
    issueScope: card.issueScope,
    rootCause: card.rootCause,
    linkedIssueId: card.issueId,
...
export async function startIngest(
  ...
  const ingestPreviewContent = preparedDocument?.analysisMarkdown ?? sourceContent
  ...
  const systemPrompt = [
    ...
    "",
    languageRule(ingestPreviewContent),
    "",
    purpose ? `## Wiki Purpose\n${purpose}` : "",
    schema ? `## Wiki Schema\n${schema}` : "",
  ...
  const userMessage = [
    ...
    `**File: ${fileName}**`,
    "```",
    ingestPreviewContent || "(empty file)",
    "```",
  ].join("\n")
