...
function unitsFromTaskContextPack(report: AgentModeReport, now: string): SemanticUnit[] {
  const pack = report.taskContextPack ?? report.compileIr.taskContextPack
  if (!pack) return []
  const units: SemanticUnit[] = []
  for (const task of pack.taskCandidates ?? []) {
    const text = [
      task.title,
      `触发：${task.trigger}`,
      `Owner：${task.ownerRole}`,
      task.collaboratorRoles.length ? `协同：${task.collaboratorRoles.join("、")}` : "",
      task.actionSteps.length ? `动作：${task.actionSteps.join("；")}` : "",
      task.acceptanceMetrics.length ? `验收：${task.acceptanceMetrics.join("；")}` : "",
    ].filter(Boolean).join("；")
    const unit = unitFromText({
      report,
      unitType: "action_recommendation",
      text,
      targetFieldKey: "task_card",
      evidenceRefs: task.sourceRefs,
      confidence: task.status === "draft" ? 0.74 : 0.56,
      now,
    })
    if (unit) {
      units.push({
        ...unit,
        businessObjectIds: [task.taskId],
      })
    }
  }
  for (const metric of pack.metricRules ?? []) {
    const unit = unitFromText({
      report,
      unitType: "metric_definition",
      text: metric.text,
      targetFieldKey: "task_metric_rule",
  ...
  for (const rule of [...(pack.collaborationRules ?? []), ...(pack.reviewRules ?? [])]) {
    const unit = unitFromText({
      report,
      unitType: "rule",
      text: rule.text,
      targetFieldKey: "task_runtime_rule",
  ...
  for (const role of pack.roleProfiles ?? []) {
    const text = [
      role.roleName,
      role.responsibilities.length ? `职责：${role.responsibilities.join("；")}` : "",
      role.kpis.length ? `KPI：${role.kpis.join("；")}` : "",
      role.collaboratorRoles.length ? `协同：${role.collaboratorRoles.join("、")}` : "",
    ].filter(Boolean).join("；")
    const unit = unitFromText({
      report,
      unitType: "claim",
      text,
      targetFieldKey: "role_profile",
...
export function extractSemanticUnitsFromReport(report: AgentModeReport, generatedAt = nowIso()): SemanticUnit[] {
  ...
  units.push(...unitsFromTaskContextPack(report, generatedAt))
