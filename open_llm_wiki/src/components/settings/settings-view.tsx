...
import { primeRuntimeEnv } from "@/lib/runtime-env"
...
import { PdfSection } from "./sections/pdf-section"
...
const CATEGORIES: Category[] = [
  ...
  { id: "pdf", labelKey: "settings.categories.pdf", icon: ImageIcon },
...
function initialDraft(
  ...
  pdfBackendMode: ReturnType<typeof useWikiStore.getState>["pdfBackendMode"],
  ...
  return {
    ...
    multimodalVisionCaptionMode: multimodal.visionCaptionMode ?? "auto_if_available",
    pdfBackendMode,
    searchProvider: search.provider,
    searchApiKey: search.apiKey,
    researchProvider: search.researchProvider ?? "firecrawl",
    firecrawlApiKey: search.firecrawlApiKey ?? "",
    firecrawlBaseUrl: search.firecrawlBaseUrl ?? "https://api.firecrawl.dev",
    outputLanguage,
    maxHistoryMessages,
...
export function SettingsView() {
  ...
  const pdfBackendMode = useWikiStore((s) => s.pdfBackendMode)
  const setPdfBackendMode = useWikiStore((s) => s.setPdfBackendMode)
  ...
  const handleSave = useCallback(async () => {
    ...
    const newSearch = {
      ...
      firecrawlApiKey: draft.firecrawlApiKey,
      firecrawlBaseUrl: draft.firecrawlBaseUrl.trim() || "https://api.firecrawl.dev",
      runtimeFirecrawlApiKey: searchApiConfig.runtimeFirecrawlApiKey ?? "",
    }
    const newEmbed = {
    ...
    const newMultimodal = {
      ...
      visionCaptionMode: draft.multimodalVisionCaptionMode ?? "auto_if_available",
    }
    ...
    primeRuntimeEnv("FIRECRAWL_API_KEY", draft.firecrawlApiKey || searchApiConfig.runtimeFirecrawlApiKey || null)
    ...
    setPdfBackendMode(draft.pdfBackendMode)
    await savePdfBackendMode(draft.pdfBackendMode)
  ...
  const body = useMemo(() => {
    switch (active) {
      ...
      case "pdf":
        return <PdfSection draft={draft} setDraft={setDraft} />
      case "web-search":
