...
export function MultimodalSection({ draft, setDraft }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold">{t("settings.sections.multimodal.title", "文档多模态理解")}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t(
            "settings.sections.multimodal.description",
            "默认对 PDF / DOCX / XMIND 开启图片与证据理解。视觉模型可用时会补充图像描述；不可用时仍保留结构解析、文本理解和 Schema 编译，只是明确标记为降级结果。",
          )}
        </p>
      </div>
      ...
      >
        <div className="min-w-0 flex-1">
            ...
            {t("settings.sections.multimodal.enableLabel", "导入时启用文档多模态理解")}
            </div>
          <div className="text-xs text-muted-foreground">
            {t(
              "settings.sections.multimodal.enableHint",
              "关闭后仍会继续解析文本，但不会为 PDF / DOCX / XMIND 的图片证据补充视觉描述。开启后，如果视觉模型可用，会自动补充图像理解；不可用时会降级继续。",
            )}
          </div>
        </div>
        <button
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                {t(
                  ...
                  "复用主模型作为视觉理解模型。仅当该模型支持图片输入时才建议开启；如果不支持，系统会自动降级并继续结构化流程。",
                )}
              </div>
            </div>
            <button
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t(
                    ...
                    "建议填写视觉模型。若当前模型不支持图片输入，系统会保留文档主链并标记“图片理解未启用，当前结果已降级”。",
                  )}
                </p>
              </div>
            </div>
            ...
            <ul className="ml-4 list-disc space-y-1 text-xs text-muted-foreground">
              <li>
                {t(
                  ...
                  "每张新图片可能触发一次视觉理解调用；相同图片会按 hash 缓存，避免重复成本。",
                )}
              </li>
              <li>
                {t(
