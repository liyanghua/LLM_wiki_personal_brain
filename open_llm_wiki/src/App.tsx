...
import { getLastProject, getRecentProjects, saveLastProject, loadLlmConfig, loadLanguage, loadSearchApiConfig, loadEmbeddingConfig, loadMultimodalConfig, loadOutputLanguage, loadProviderConfigs, loadActivePresetId, loadProxyConfig, loadPdfBackendMode } from "@/lib/project-store"
import { loadReviewItems, loadChatHistory } from "@/lib/persist"
import { setupAutoSave } from "@/lib/auto-save"
import { startClipWatcher } from "@/lib/clip-watcher"
import { ensureScenePack } from "@/lib/scene-pack"
import { upgradeProjectDocsIfLegacy } from "@/lib/project-doc-upgrade"
import {
  loadAgentModeReports,
  loadAgentLoopSessions,
  loadInteractiveSessions,
  loadRevisionDrafts,
  loadRevisionVersions,
} from "@/lib/agent-mode-persist"
import { loadResearchSessions } from "@/lib/research-persist"
...
import { readRuntimeEnv, primeRuntimeEnv } from "@/lib/runtime-env"
...
function App() {
    ...
    async function init() {
      try {
        ...
        if (savedSearchConfig) {
          const runtimeFirecrawlApiKey = await readRuntimeEnv("FIRECRAWL_API_KEY")
          primeRuntimeEnv("FIRECRAWL_API_KEY", runtimeFirecrawlApiKey)
          useWikiStore.getState().setSearchApiConfig({
            ...savedSearchConfig,
            runtimeFirecrawlApiKey:
              savedSearchConfig.runtimeFirecrawlApiKey?.trim() || runtimeFirecrawlApiKey || "",
          })
        } else {
          const runtimeFirecrawlApiKey = await readRuntimeEnv("FIRECRAWL_API_KEY")
          primeRuntimeEnv("FIRECRAWL_API_KEY", runtimeFirecrawlApiKey)
          if (runtimeFirecrawlApiKey) {
        ...
        const savedPdfBackendMode = await loadPdfBackendMode()
        if (savedPdfBackendMode) {
          useWikiStore.getState().setPdfBackendMode(savedPdfBackendMode)
        }
  ...
  async function handleProjectOpened(proj: WikiProject) {
    ...
    await upgradeProjectDocsIfLegacy(proj.path)
    ...
    try {
      ...
      const [reports, loops, sessions, drafts, versions] = await Promise.all([
        loadAgentModeReports(proj.path),
        loadAgentLoopSessions(proj.path),
        loadInteractiveSessions(proj.path),
        loadRevisionDrafts(proj.path),
        loadRevisionVersions(proj.path),
      ])
      useAgentModeStore.getState().setReports(reports)
      useAgentLoopStore.getState().setSessions(loops)
      useInteractiveSessionStore.getState().setSessions(sessions)
      useRevisionStore.getState().setDrafts(drafts)
      useRevisionStore.getState().setVersions(versions)
    } catch {
      // ignore, start fresh
    }
    try {
      const researchSessions = await loadResearchSessions(proj.path)
      useResearchStore.getState().setSessions(researchSessions)
    } catch {
      // ignore, start fresh
    }
  }
