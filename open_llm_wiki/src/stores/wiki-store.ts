...
export type VisionCaptionMode = "auto_if_available"

export interface SearchApiConfig {
  provider: "tavily" | "firecrawl" | "none"
  apiKey: string
  researchProvider?: "tavily" | "firecrawl"
  firecrawlApiKey?: string
  firecrawlBaseUrl?: string
  runtimeFirecrawlApiKey?: string
}
...
export interface WikiState {
  ...
  setAgentWorkbenchBottomHeight: (height: number) => void
  setAgentWorkbenchQaExpanded: (expanded: boolean) => void
  ...
  setPdfBackendMode: (mode: PdfBackendMode) => void
...
export const useWikiStore = create<WikiState>((set) => ({
  ...
  chatExpanded: false,
  agentWorkbenchBottomHeight: 460,
  agentWorkbenchQaExpanded: true,
  activeView: "wiki",
  llmConfig: {
    provider: "openai",
    apiKey: "",
  ...
  setAgentWorkbenchBottomHeight: (agentWorkbenchBottomHeight) => set({ agentWorkbenchBottomHeight }),
  setAgentWorkbenchQaExpanded: (agentWorkbenchQaExpanded) => set({ agentWorkbenchQaExpanded }),
  ...
  multimodalConfig: {
    ...
    concurrency: 4,
    visionCaptionMode: "auto_if_available",
  },

  pdfBackendMode: "opendataloader",
  ...
  setPdfBackendMode: (pdfBackendMode) => set({ pdfBackendMode }),
  setOutputLanguage: (outputLanguage) => set({ outputLanguage }),
  setProxyConfig: (proxyConfig) => set({ proxyConfig }),
  bumpDataVersion: () => set((state) => ({ dataVersion: state.dataVersion + 1 })),
}))

export type { EmbeddingConfig, MultimodalConfig, OutputLanguage, ProxyConfig }
